...
class Frontier_Explorer(Node):
    def __init__(self):
        ...
        self.state_sub = self.create_subscription(TurtleBotState, "/state", self.state_callback, 10) # TODO make subscriber node
        self.map_sub = self.create_subscription(OccupancyGrid, "/map", self.map_callback, 10)# TODO make subscriber node
        self.nav_success_sub = self.create_subscription(Bool, "/nav_success", self.nav_success_cb, 10) # TODO make subscriber node
    ...
    def explore(self):
        ...
        unknown_mask = np.where(self.occupancy.probs ==-1, 1, 0 )# TODO use np.where to find unknown mask which is indicated by the value -1
        ...
        kernel = np.ones((window_size, window_size)) / (window_size **2)# TODO define the kernel
        occupied= convolve2d(unknown_mask.astype(float), kernel, mode='same')# TODO 2d convolution of mask (use convolve2d) using kernel and mode='same'
        unoccupied=convolve2d(occupied_mask.astype(float), kernel, mode='same')# TODO 2d convolution of mask (use convolve2d) using kernel and mode='same'
        unknown= convolve2d(unoccupied_mask.astype(float), kernel, mode='same')# TODO 2d convolution of mask (use convolve2d) using kernel and mode='same'
        ...
        msg = self.state
        msg.x, msg.y = float(frontier_state[0]), float(frontier_state[1])#TODO
        self.cmd_nav_pub.publish(self.state) # TODO below publish frontier state into cmd_nav topic
